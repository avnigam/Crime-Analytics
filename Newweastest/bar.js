var global_year_bar = 0;
var global_state_bar = "";
var global_choice_bar = "";

function bar_plot(plotdata, state, year) {
...
function bar(choice, state, year) {
	...
	console.log('http://localhost:5000/api/bar/' + year + '/' + state + '/' + choice);
	
    d3.json('http://localhost:5000/api/bar/' + year + '/' + state + '/' + choice, function (error, data) {
        var csv = ConvertToCSV(data);
        bar_plot(csv, state, year);
    });
}
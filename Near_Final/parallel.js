var global_year_parallel = 0;
var global_state_parallel = "";

function parallel_plot(plotdata, state, year) {
...
function parallel_csv(objArray) {
    ...
    var str = '';
	str = 'perpetrator age,perpetrator race,perpetrator sex,relationship,victim age,victim race,victim sex,crime solved' + '\r\n';
...
function parallel(state, year) {
	...
	if (state == '') {
		state = global_state_parallel;
	} else {
		global_state_parallel = state;
	}
	
	if (year == 0) {
		year = global_year_parallel;
	} else {
		global_year_parallel = year;
	}
    d3.json('http://localhost:5000/api/parallel/' + year + '/' + state, function (error, data) {
        var csv = parallel_csv(data);
        parallel_plot(csv, state, year);
    });
}
...
function showHeatMapPlot(svg, year, plotdata) {
	...
	d3.csv("homicide.csv", function(err, data) {
	  ...
	  data.forEach(function(d) {
		if(d[MAP_YEAR] == year) {
			if (year != current_year){
				...
				current_year = year;
				bar('', current_state, current_year);
				parallel(current_state, current_year);
				line_new(current_state, current_year);
			} 
		...
		svg.append("g")
		  ...
		  .enter().append("path")
			...
			.on("mousedown", function(d) {
				current_state = id_name_map[d.id];
				console.log(current_state);
				stackedChart('',current_state);
				bar('',current_state, current_year);
				parallel(current_state, current_year);
				line_new(current_state, current_year);
			})
